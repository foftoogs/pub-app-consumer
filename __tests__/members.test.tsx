...
describe('MembersScreen', () => {
  ...
  it('shows add input when add button pressed', () => {
    ...
    expect(getByPlaceholderText('Consumer ID')).toBeTruthy();
    expect(getByText('Add')).toBeTruthy();
    expect(getByText('Cancel')).toBeTruthy();
  });

  it('calls addMember API on submit', async () => {
    const newMember = {
      id: 'member-3',
    ...
    (mockApi.post as jest.Mock).mockResolvedValueOnce({ data: { member: newMember } });

    const { getByText, getByPlaceholderText } = render(<MembersScreen />);
    fireEvent.press(getByText('+ Add Member'));
    fireEvent.changeText(getByPlaceholderText('Consumer ID'), 'consumer-3');
  ...
  it('shows error on failed add', async () => {
    (mockApi.post as jest.Mock).mockRejectedValueOnce({
      response: { data: { message: 'Consumer not found' } },
    });

    const { getByText, getByPlaceholderText, findByText } = render(<MembersScreen />);
    fireEvent.press(getByText('+ Add Member'));
    fireEvent.changeText(getByPlaceholderText('Consumer ID'), 'bad-id');
    fireEvent.press(getByText('Add'));

    expect(await findByText('Consumer not found')).toBeTruthy();
  });
